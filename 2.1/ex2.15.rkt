...
#|
Eva is right. The fewer times we use a variable the less we create errors like the one above
where a/a is not centered on 1 but is just a bit off. And in terms of errors, we will also end up
with a tighter interval since we are not multiplying tolerances (or adding them in this case)
this is why par2 is a better program because it produces a tighter interval that is more accurate
|#