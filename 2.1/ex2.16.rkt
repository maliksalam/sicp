...
#|
This is an example of why two expressions might be agebraicly equivalent but might not yield the
same result. This is inherrent to algebra with uncertain values (intervals) the more the value is
used the more the uncertainty grows. If we could represent every function with only one iteration
of a variable, this could work but in general this is not possible. (for example (A+B)/(A+C))
Moreover expressions that are equivalent for simple real numbers might not be for uncertain values
we have seen the identity error term balloons up the more we use the variable. So simple things
like simplifying fractions cannot be easily translated to interval algebra.
The way to solve this is either with Monte Carlo method (simulation) or by analytical methods
(getting the span of the desired function)
|#